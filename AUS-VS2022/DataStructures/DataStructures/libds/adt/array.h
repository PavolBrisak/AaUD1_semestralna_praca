...
namespace ds::adt {
	...
	T CompactMatrix<T>::access(long long index1, long long index2) const
	{
		...
		size_t mapovanyIndex = mapIndices(index1, index2);
		return getSequence()->access(mapovanyIndex)->data_;
	}
	template<typename T>
	void CompactMatrix<T>::set(T element, long long index1, long long index2)
	{
		...
		size_t mapovanyIndex = mapIndices(index1, index2);
		getSequence()->access(mapovanyIndex)->data_ = element;
	}
	...
	{
		return (index1 = dimension1_.getBase()) * dimension2_.getSize() + (index1 - dimension2_.getBase());
	}
