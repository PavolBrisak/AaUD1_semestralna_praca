...
namespace ds::adt {
    ...
    {
        if (size() == getSequence()->size())
        {
            this->error("Kapacita frontu je naplnena!");
        }
        getSequence()->access(insertionIndex_)->data_ = element;
    ...
    T& ImplicitQueue<T>::peek()
    {
        if (this->isEmpty())
        {
            throw std::out_of_range("Front je prazdny");
        }
    ...
    T ImplicitQueue<T>::pop()
    {
        ...
        T vysledok = getSequence()->access(insertionIndex_)->data_;
        insertionIndex_ = getSequence()->indexOfNext(insertionIndex_);
        --size_;
        return vysledok;
    }
